...
class OceanDrawer:
    ...
    def draw_grid(self):
        ...
        for x in self.velocity_grid_x:
            ...
            pygame.draw.line(
                self.screen, grid_color, (sx, 0), (sx, self.screen_height)
            )
        # Draw horizontal grid lines.
        for y in self.velocity_grid_y:
            _, sy = self.sim_to_screen(0, y)
            pygame.draw.line(
                self.screen, grid_color, (0, sy), (self.screen_width, sy)
            )
    ...
    def draw_velocity_vectors(self, velocities):
        ...
        for i, x in enumerate(self.velocity_grid_x):
            for j, y in enumerate(self.velocity_grid_y):
                ...
                pygame.draw.line(
                    self.screen, arrow_color, (sx, sy), (ex, ey), 2
                )
    ...
    def run(self):
        ...
        while running:
            ...
            X_grid, Y_grid = np.meshgrid(
                self.velocity_grid_x, self.velocity_grid_y, indexing="ij"
            )
